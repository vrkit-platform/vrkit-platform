...
void process()
{
	...
	const float steeringWheelTorque = idk.getVarFloat(steeringWheelTorque_idx);

	const int torqBin = static_cast<int>(fabsf(steeringWheelTorque));
...
void shutdown()
{
	...
	else
	{
		...
		{
			const float pct = (float)hist[i] / (float)histCount;
			pctTot += pct;
