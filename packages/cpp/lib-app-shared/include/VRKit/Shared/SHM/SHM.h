...
#include <VRKit/Shared/Constants.h>
#include <VRKit/Shared/FileSystemHelpers.h>
#include <VRKit/Shared/Geometry2D.h>
#include <VRKit/Shared/VRTypes.h>
#include <VRKit/Shared/Macros.h>
#include <VRKit/Shared/ScreenTypes.h>
...
namespace VRKit::Shared::SHM {
  ...
  struct IPCHandles {
    ...
    IPCHandles(HANDLE feederProcess, const FrameMetadata& frame)
      : foreignTextureHandle(frame.texture),
        foreignFenceHandle(frame.fence) {
      const auto thisProcess = GetCurrentProcess();
      winrt::check_bool(
        DuplicateHandle(feederProcess, frame.fence, thisProcess, fenceHandle.put(), NULL, FALSE, DUPLICATE_SAME_ACCESS)
      );
      winrt::check_bool(
        DuplicateHandle(
          feederProcess,
    ...
    public:
      ...
      T* getTexture() const {
        if (state_ != State::ValidWithTexture) [[unlikely]] {
          VRK_LOG_AND_FATAL("Called SHM::Snapshot::GetTexture() with invalid state {}", static_cast<uint8_t>(state_));
        }
      ...
      constexpr bool hasMetadata() const {
        return state_ == State::ValidWithoutTexture || state_ == State::ValidWithTexture;
      }
  ...
  class SHMCachedReader : public SHMReader {
    public:
      ...
      virtual Snapshot maybeGet(); //const std::source_location& loc = std::source_location::current()
    ...
    private:
      ...
      std::shared_ptr<IPCClientTexture> getIPCClientTexture(const PixelSize&, uint8_t swapchainIndex) noexcept;
