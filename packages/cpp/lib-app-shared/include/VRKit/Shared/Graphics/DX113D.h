...
namespace VRKit::Shared::Graphics {
  ...
  class SavedState final {
    public:
      ...
      explicit SavedState(const winrt::com_ptr<ID3D11DeviceContext>&);
      explicit SavedState(ID3D11DeviceContext*);
   ...
   * just for this
   * - hook into an existing render pipeline so can't make assumptions about the
