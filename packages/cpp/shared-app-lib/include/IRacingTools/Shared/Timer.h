...
struct Event {
    ...
    Event(TimerId id, Timestamp start, Duration period, Func &&handler) :
...
public:
    ...
    inline TimerId add(
        const std::chrono::duration<Rep, Period> &when, HandlerFn &&handler, const Duration &period = Duration::zero()
    ) {
    ...
    bool remove(TimerId id) {
        ...
        auto it = std::find_if(timeEvents_.begin(), timeEvents_.end(), [&](const detail::TimeEvent &te) {
            return te.ref == id;
        });
