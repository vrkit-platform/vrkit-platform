  ...
  styleOverrides: {
    root: ({ theme }) => ({
      [`& .${sliderClasses.thumb}`]: {
        ...
        borderColor: appAlpha(theme.palette.grey['500'], 0.08),
        '&::before': {
          opacity: 0.4,
          boxShadow: 'none',
          width: 'calc(100% - 4px)',
          height: 'calc(100% - 4px)',
          backgroundImage: `linear-gradient(180deg, ${theme.palette.grey[500]} 0%, ${appAlpha(theme.palette.grey['500'], 0)} 100%)`,
    ...
    markActive: ({ theme }) => ({
      backgroundColor: appAlpha(theme.palette.common.whiteChannel, 0.64),
    }),
    markLabel: ({ theme }) => ({
