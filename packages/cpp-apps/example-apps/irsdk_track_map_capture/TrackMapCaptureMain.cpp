...
bool parceYamlInt(const char* yamlStr, const char* path, int* dest)
{
    if (dest)
    {
        ...
        if (yamlStr && path)
        {
            ...
            if (Utils::ParseYaml(yamlStr, path, &strPtr, &count))
            {
                (*dest) = atoi(strPtr);
                return true;
            }
...
void printSessionState(int stateValue)
{
    auto state = magic_enum::enum_cast<SessionState>(stateValue).value();
    switch (state)
    {
...
void printPaceMode(int modeValue)
{
    auto mode = magic_enum::enum_cast<PaceMode>(modeValue).value();
    switch (mode)
    {
...
void updateDisplay()
{
    ...
    if (g_playerInCar.isValid()) {
        if (g_playerInCar.getBool()) {
            // if (g_SessionTime.isValid()) {
            //     printf("Time: ");
            //     printTime(g_SessionTime.getDouble());
            // }

            if (g_Lat.isValid()) {
                printf(" Lat, Lon: %f, %f\n\n", g_Lat.getDouble(), g_Lon.getDouble());
            }
        }
    }
...
{
    printf("track map capture 1.0, press any key to exit\n");
