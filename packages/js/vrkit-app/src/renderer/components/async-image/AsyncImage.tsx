...
const resToError = (res: Response) => {
  ...
  const errCode = res?.status ?? -1,
    ...
    errUrl = res?.url ?? "NULL"
  return Error(`Error (url=${errUrl},code=${errCode},text=${errText})`)
}
 ...
 */
export const AsyncImage = React.forwardRef<HTMLDivElement, AsyncImageProps>(function AsyncImage(
  ...
  const fetchImageWithCache = async () => {
    const setHTMLFallback = () => {
      if (unpackIfPossible && (isString(fallback) || !fallback)) {
        const fallbackUrl = isString(fallback) && fallback.length ? fallback : EmptyImageURL,
