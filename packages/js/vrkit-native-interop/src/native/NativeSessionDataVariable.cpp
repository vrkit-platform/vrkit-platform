...
namespace IRacingTools::App::Node {
    ...
    NativeSessionDataVariable::jsGetBool(const Napi::CallbackInfo& info) {
        ...
        if (varHolder_->isValid() && varHolder_->getType() == VarDataType::Bool)
            return Napi::Boolean::New(env, varHolder_->getBool(entry));
    ...
    Napi::Value NativeSessionDataVariable::jsGetInt(const Napi::CallbackInfo& info) {
        ...
        if (varHolder_->isValid() && varHolder_->getType() == VarDataType::Int32)
            return Napi::Number::New(env, varHolder_->getInt(entry));
    ...
    Napi::Value NativeSessionDataVariable::jsGetFloat(const Napi::CallbackInfo& info) {
        ...
        if (varHolder_->isValid() && varHolder_->getType() == VarDataType::Float)
            return Napi::Number::New(env, varHolder_->getFloat(entry));
