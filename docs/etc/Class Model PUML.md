...
namespace IRacingTools {
	namespace Shared {
		class DiskSessionDataProvider {
			+DiskSessionDataProvider(const std::filesystem::path& file, IRacingSDK::ClientId clientId)
		...
		class SessionDataAccess {
			...
			+SessionDataAccess(std::weak_ptr<IRacingSDK::Client> client)
			+getClient() : std::shared_ptr<IRacingSDK::Client>
			-client_ : std::weak_ptr<IRacingSDK::Client>
		}
		...
		class SessionDataUpdatedDataEvent {
			...
			+sessionInfo() : std::weak_ptr<IRacingSDK::SessionInfo::SessionInfoMessage>
			+refresh() : void
		}
