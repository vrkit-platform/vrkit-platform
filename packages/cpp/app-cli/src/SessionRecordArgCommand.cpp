...
void SessionRecordArgCommand::configure(CLI::App *app) {


  auto cmd = app->add_subcommand("record", "Record Live Session");

  cmd->add_option("-o,--output", outputPath_, "Output path")->required(false)->default_val(std::filesystem::current_path().string());
