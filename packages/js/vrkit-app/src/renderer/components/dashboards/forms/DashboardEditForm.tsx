...
import {
  FormActionFooterDefault, FormContainer, FormContent, FormRow
} from "../../form"
import { get } from "lodash/fp"
// import { useActionContainer } from "../../../../hooks"
import { actionFromTemplate, isEqual, isModelSameOrUpdated } from "vrkit-shared"
import { useModelEditorContext } from "../../model-editor-context"
import { bind, cloneInstanceOf, isNotEmpty, propEqualTo } from "vrkit-shared/utils"
 ...
 */
const DashboardEditInnerForm = withFormik<
    DashboardEditInnerFormProps,
    DashboardConfig
>({
  ...
  handleSubmit: (
      ...
      formikBag: FormikBag<DashboardEditInnerFormProps, DashboardConfig>
  ) => formikBag.props.onSubmit(values, formikBag)
})(function DashboardEditInnerForm(
    props: DashboardEditInnerFormProps & FormikProps<DashboardConfig>
) {
  const {
        errors,
        handleBlur,
  ...
  useEffect(() => {
    const updatedModel = cloneInstanceOf(DashboardConfig as any, model, values)
    if (
        !isEqual(updatedModel, model)
        // !attributesEqual(updatedModel, model, Project) &&
  ...
  //             .filter(isNotEmpty)
  //             .getOrCall(() => name.substring(0, Math.min(name.length, 4)))
  //
  //     info("Generated project code ", newCode, "from name", name)
  //     setImmediate(() => {
  //       setFieldValue("code", newCode, false)
  //     })
  //   }
  ...
  return (
      <FormContainer noValidate onSubmit={handleSubmit}>
        <FormContent>
          <FormRow justifyContent="center" mt={6} mb={2}>
            {/*<ProjectIconEditor*/}
          ...
          {/* TAGS & MILESTONES */}
          <FormRow>
            {/*<ProjectTagListField project={values} />*/}
            {/*<ProjectMilestoneListField project={values} />*/}
          </FormRow>
...
export function DashboardEditForm(props: DashboardEditFormProps) {
  const isMounted = useIsMounted(),
      ...
      sourceDash =
          models.find(propEqualTo("id", mutatingDash?.id)) ?? mutatingDash,
      
      dash = useMemo(
          () => DashboardConfig.create(sourceDash),
          [sourceDash?.id]
      ),
      handleSubmitRef = useRef<Function>(),
      dispatch = useAppDispatch(),
      onReset = useMemo(
          () => bind(resetMutatingModels, null, [mutatingDash?.id]),
          [mutatingDash?.id]
      ),
      onSubmit = Alert.usePromise(
          async (
              values: DashboardConfig,
              {
                ...
                setStatus,
                setSubmitting
              }: FormikContextType<DashboardConfig>
          ) => {
          ...
          },
          {
            ...
            success: ({ result }) =>
                `"Successfully saved project (${result.name})."`,
