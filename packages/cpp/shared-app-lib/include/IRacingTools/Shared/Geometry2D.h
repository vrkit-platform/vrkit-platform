...
namespace IRacingTools::Shared {
  ...
  enum class ScaleToFitMode {
    ShrinkOrGrow, GrowOnly, ShrinkOnly,
  };
  ...
  template<class T> struct Size {
    ...
    constexpr Size<T> operator*(
        const std::floating_point auto operand) const noexcept requires std::floating_point<T> {
      return {width_ * operand, height_ * operand};
    }
    ...
    constexpr Size<T> scaledToFit(
        const Size<T> &container, ScaleToFitMode mode = ScaleToFitMode::ShrinkOrGrow) const noexcept {
      ...
      const Size<float> scaled{width_ * scale, height_ * scale,};
    ...
    constexpr Size<T> integerScaledToFit(
      ...
      if (scale > 1) {
        ...
        return {width_ * mult, height_ * mult,};
      }
    ...
    template<std::integral TValue, class TSize = Size<TValue>> constexpr TSize floor() const noexcept {
      return TSize{static_cast<TValue>(std::floor(width_)), static_cast<TValue>(std::floor(height_)),};
    }

    template<class TValue, class TSize = Size<TValue>> requires std::floating_point<T>
    constexpr TSize rounded() const noexcept {
      return {static_cast<TValue>(std::lround(width_)), static_cast<TValue>(std::lround(height_)),};
    }

    constexpr operator D2D1_SIZE_U() const requires std::integral<T> {
  ...
  template<class T> struct Point {
    ...
    template<class TV = T> constexpr auto x() const {
      return static_cast<TV>(x_);
    }

    template<class TV = T> constexpr auto y() const {
    ...
    friend constexpr Point<T> operator+(
        Point<T> lhs, const Point<T> &rhs) noexcept {
      lhs += rhs;
      return lhs;
    }
    ...
    template<class TValue, class TPoint = Point<TValue>> constexpr TPoint staticCast() const noexcept {
      return TPoint{static_cast<TValue>(x_), static_cast<TValue>(y_),};
    }

    template<class TValue, class TPoint = Point<TValue>> requires std::floating_point<T>
    constexpr TPoint rounded() const noexcept {
      return {static_cast<TValue>(std::lround(x_)), static_cast<TValue>(std::lround(y_)),};
    }
    ...
    constexpr operator D2D1_POINT_2U() const noexcept requires std::integral<T> {
  ...
  template<class T> struct Rect {
    ...
    constexpr Rect<T> operator/(const T divisor) const noexcept {
      return {offset_ / divisor, size_ / divisor,};
    }

    template<std::integral TValue> constexpr Rect<T> operator*(const TValue operand) const noexcept {
      return {offset_ * operand, size_ * operand,};
    }

    template<std::floating_point TValue> requires std::floating_point<T>
    constexpr Rect<T> operator*(const TValue operand) const noexcept {
      return {offset_ * operand, size_ * operand,};
    }

    template<class TV = T> constexpr auto left() const {
      return offset_.template x<TV>();
    }

    template<class TV = T> constexpr auto top() const {
      return offset_.template y<TV>();
    }

    template<class TV = T> constexpr auto right() const {
      return left<TV>() + size_.width_;
    }

    template<class TV = T> constexpr auto bottom() const {
    ...
    constexpr Rect<T> withOrigin(Origin otherOrigin, const Size<T> &container) const {
      if (origin_ == otherOrigin) {
        return *this;
      }
      return {{offset_.x_, container.height_ - offset_.y_}, size_, origin_,};
    }

    constexpr auto operator<=>(const Rect<T> &) const noexcept = default;

    template<class TValue, class TRect = Rect<TValue>> constexpr TRect staticCast() const noexcept {
      return {{static_cast<TValue>(offset_.x_),   static_cast<TValue>(offset_.y_),},
              {static_cast<TValue>(size_.width_), static_cast<TValue>(size_.height_),},};
    }

    template<class TValue, class TRect> constexpr TRect staticCastWithBottomRight() const noexcept {
      return {static_cast<TValue>(offset_.x_), static_cast<TValue>(offset_.y_), static_cast<TValue>(offset_.x_ + size_.width_),
              static_cast<TValue>(offset_.y_ + size_.height_),};
    }

    template<class TValue> requires std::floating_point<T>
    constexpr Rect<TValue> rounded() const noexcept {
      return {offset_.template rounded<TValue>(), size_.template rounded<TValue>()};
    }

    constexpr operator D3D11_RECT() const requires std::integral<T> {
      return staticCastWithBottomRight<LONG, D3D11_RECT>();
    }

    constexpr operator D2D_RECT_U() const requires std::integral<T> {
...
template<typename T>
  ...
  template<class T> void from_json(const nlohmann::json &j, Size<T> &v) {
    v.width_ = j.at("width");
    v.height_ = j.at("height");
  }

  template<class T> void to_json(nlohmann::json &j, const Size<T> &v) {
    j["width"] = v.width_;
    j["height"] = v.height_;
  }
