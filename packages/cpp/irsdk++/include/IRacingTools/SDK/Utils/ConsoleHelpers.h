...
size_t GetStringViewMaxLength(const std::array<std::string_view, ColumnCount> &headers) {
    auto res = std::ranges::max_element(headers, [](const auto &a, const auto &b) { return a.length() < b.length(); });
    return (*res).length();
}
...
void PrintTabularData(
    std::array<std::string_view, ColumnCount> headers,
    std::list<std::tuple<ColumnTypes...>> rows) {
