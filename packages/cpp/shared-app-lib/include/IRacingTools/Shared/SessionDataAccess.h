...
namespace IRacingTools::Shared {
  class SessionDataAccess : public IRacingTools::SDK::ClientProvider {
  private:
    std::weak_ptr<IRacingTools::SDK::Client> client_;

  public:
    SessionDataAccess() = delete;

    explicit SessionDataAccess(std::weak_ptr<SDK::Client> client) : client_(std::move(client)){};

    SessionDataAccess(const SessionDataAccess &) = delete;

    SessionDataAccess(SessionDataAccess &&) = delete;

    virtual std::shared_ptr<IRacingTools::SDK::Client> getClient() override;
